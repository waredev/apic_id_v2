<template>
    <div class="xl:px-20 px-8 font-arian-bold xl:pt-60 pt-32 pb-20">
        <div class="xl:text-4xl text-2xl">{{ $t('about.supporting_professional_institutions') }}</div>
        ...
        <div v-if="!loading && data" class="mb-5 mt-8 flex">
            <a target="_blank" :href="data" class="w-auto h-12 mt-1 border group hover:bg-primary hover:border-primary cursor-pointer flex items-center px-3 rounded-md">
                ...
                <div class="group-hover:text-white font-arian-demi ml-2 mx-3">{{ $t('about.supporting_professional_institutions') }}</div>
...
<script>
    export default {
        ...
        methods: {
            async fetchData(){
                const res = await this.$store.dispatch('fetchSupportingProfessionalInstitution');
