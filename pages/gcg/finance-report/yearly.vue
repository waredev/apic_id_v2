<template>
    <div>
        <div class="xl:px-20 px-8 font-arian-bold xl:pt-60 pt-32 pb-20">
            <div class="xl:text-4xl text-2xl">{{ $t('gcg.financial_report_yearly') }}</div>
            <div class="p-5 border rounded-md mt-10">
                ...
                <div v-if="!loading && data.length > 0" v-for="(item, index) in data" :key="index" class="mb-5">
                    ...
                    <a target="_blank" :href="$axios.defaults.baseURL + item.attributes.file.data.attributes.url" class="w-96 h-12 mt-1 border group hover:bg-primary hover:border-primary cursor-pointer flex items-center px-3 rounded-md">
                        ...
                        <div class="group-hover:text-white font-arian-demi ml-2">{{item.attributes.label.length > 28 ? item.attributes.label.substring(0, 30) + '...' : item.attributes.label}}</div>
...
<script>
    export default {
        name: 'FinancialReportYearlyPage',
        data(){
            return {
                data: [],
                loading: true,
        ...
        methods: {
            async fetchData(){
                const res = await this.$store.dispatch('fetchFiancialReportYearly');
