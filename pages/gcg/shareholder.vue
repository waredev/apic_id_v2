<template>
    <div class="xl:px-20 px-8 font-arian-bold xl:pt-60 pt-32 pb-20">
        <div class="xl:text-4xl text-2xl">{{ $t('gcg.shareholder') }}</div>
        ...
        <div v-if="!loading && data" class="mb-5 mt-8">
            <a target="_blank" :href="data" class="xl:w-96 w-full h-12 mt-1 border group hover:bg-primary hover:border-primary cursor-pointer flex items-center px-3 rounded-md">
...
<script>
    export default {
        ...
        methods: {
            async fetchData(){
                const res = await this.$store.dispatch('fetchShareholder');
