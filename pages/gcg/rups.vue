            ...
            <div class="p-5 border rounded-md mt-10">
                ...
                <div>
                    ...
                    <div v-if="!loading" v-for="(item, index) in links" :key="index" class="mb-5">
                        ...
                        <a target="_blank" :href="item.link" class="w-96 h-12 mt-1 border group hover:bg-primary hover:border-primary cursor-pointer flex items-center px-3 rounded-md">
                            <img src="~/assets/images/export.png" class="w-6" alt="pdf" />
                            <div class="group-hover:text-white font-arian-demi ml-2">{{item.label.length > 28 ? item.label.substring(0, 30) + '...' : item.label}}</div>
                ...
                <div class="mt-10">
                    ...
                    <div v-if="!loading" v-for="(item, index) in downloads" :key="index" class="mb-5">
                        ...
                        <a target="_blank" :href="$axios.defaults.baseURL + item.file.data.attributes.url" class="w-96 h-12 mt-1 border group hover:bg-primary hover:border-primary cursor-pointer flex items-center px-3 rounded-md">
